...
export default function AboutSection() {
  ...
  return (
    <section 
      id="about" 
      ref={containerRef}
      className="sticky top-0 h-screen w-full bg-gradient-to-br from-[#05040f] via-[#0c0d24] to-[#0a0718] overflow-hidden text-white flex items-center justify-center px-6 select-none z-20 shadow-[0_-40px_80px_rgba(0,0,0,0.9)]"
    ...
    >
      ...
      <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 relative z-10 items-center">
        ...
        <div className="lg:col-span-5 flex flex-col gap-8 order-2 lg:order-1 text-right" dir="rtl">
          ...
          <div className="reveal-about-item grid grid-cols-3 gap-4 pt-6 border-t border-white/5 font-mono">
            <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-4 text-center">
              <span className="block text-xl font-bold bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">+۴۰</span>
              <span className="text-[9px] text-white/40 uppercase tracking-wider block mt-1">پروژه موفق</span>
            </div>
            <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-4 text-center">
              <span className="block text-xl font-bold bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent">۹۹.۹٪</span>
              <span className="text-[9px] text-white/40 uppercase tracking-wider block mt-1">پایداری فرکانس</span>
            </div>
            <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-4 text-center">
              <span className="block text-xl font-bold bg-gradient-to-r from-pink-400 to-orange-500 bg-clip-text text-transparent">۳</span>
        ...
        <div className="lg:col-span-7 w-full flex flex-col md:flex-row gap-4 items-stretch justify-center order-1 lg:order-2">
          {team.map((member, idx) => {
            ...
            return (
              <div
                ...
                className={`reveal-about-item relative overflow-hidden rounded-[2.5rem] border cursor-pointer transition-all duration-700 ease-[cubic-bezier(0.25,1,0.3,1)]
                  ${isActive 
                    ? "flex-[3.5] h-[350px] md:h-[500px] border-[var(--accent-color)] shadow-[0_20px_50px_rgba(0,0,0,0.6)]" 
                    : "flex-[1] h-[95px] md:h-[500px] border-white/10 opacity-60 hover:opacity-100"
                  }`}
              >
                {isActive && (
                ...
                <div 
                  className={`absolute inset-0 flex flex-col justify-end p-6 md:p-8 text-right transition-all duration-500 ease-out
